...
public class IPostRiskAdmissionsCalculateIncomesMapperTest {
    ...
    public void testMapInDataWithNulls() {
        Body bodyNull = null; // Setting the input to null
        BBody bBodyNullResult = mapper.mapInData(bodyNull);
        Assert.assertNull(bBodyNullResult); // Expecting null output

        Body body = new Body();
        BBody bBody = mapper.mapInData(body);
        Assert.assertNotNull(bBody); // Expecting a non-null object
        Assert.assertNull(bBody.getPerson()); // Expecting internal objects to be null
    ...
    public void testMapOutData() {
        ...
        //Similar asserts as testMapInData, adapted for mapOutData
    ...
    private class DummyMock {
        ...
        public BBody getBBody() {
           // Analogous to getBody(), creating a BBody instance with data
           // ... (implementation for creating BBody instance)
            BBody bBody = new BBody();
