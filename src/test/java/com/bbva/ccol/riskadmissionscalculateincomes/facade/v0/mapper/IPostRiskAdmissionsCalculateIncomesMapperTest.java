...
public class IPostRiskAdmissionsCalculateIncomesMapperTest {
    ...
    public void testMapInData() {
        ...
        Assert.assertEquals(Double.valueOf(body.getPerson().getLaboralInformation().getDeclaratedIncome()), bBody.getPerson().getLaboralInformation().getDeclaratedIncome());

        List<InformationSources> informationSources = body.getInformationSources();
        List<BInformationSources> bInformationSources = bBody.getInformationSources();
        for (int i = 0; i < informationSources.size(); i++) {
            Assert.assertEquals(informationSources.get(i).getId(), bInformationSources.get(i).getId());
            Assert.assertEquals(informationSources.get(i).getIsConsulted(), bInformationSources.get(i).getIsConsulted());
        }
    ...
    public void testMapOutData() {
        ...
        Assert.assertEquals(Long.valueOf(bBody.getPerson().getLaboralInformation().getDeclaratedIncome()), body.getPerson().getLaboralInformation().getDeclaratedIncome());

        List<BInformationSources> bInformationSources = bBody.getInformationSources();
        List<InformationSources> informationSources = body.getInformationSources();
        for (int i = 0; i < bInformationSources.size(); i++) {
            Assert.assertEquals(bInformationSources.get(i).getId(), informationSources.get(i).getId());
            Assert.assertEquals(bInformationSources.get(i).getIsConsulted(), informationSources.get(i).getIsConsulted());
        }
