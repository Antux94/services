...
public class ICalculateV0MapperTxTest {
    ...
    public void testMapInApxDataWithNulls() {
        RequestTransactionCmlct001_1 request = null;
        BBody response = mapper.mapInApxData(request);
        Assert.assertNull(response);


        request = new RequestTransactionCmlct001_1();
        ...
        request = dummyMock.getRequestTransactionCmlct001_1WithNulls();
        response = mapper.mapInApxData(request);

        Assert.assertNotNull(response);
        Assert.assertNull(response.getPerson().getIdentityDocument());
        Assert.assertNull(response.getPerson().getLaboralInformation());
        Assert.assertTrue(response.getInformationSources().isEmpty());
